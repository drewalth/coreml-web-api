...
class Resnet50Input: MLFeatureProvider {
    ...
    convenience init(imageWith image: CGImage) throws {
        try self.init(image: MLFeatureValue(cgImage: image, pixelsWide: 224, pixelsHigh: 224, pixelFormatType: kCVPixelFormatType_32BGRA, options: nil).imageBufferValue!)
    }

    @available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    convenience init(imageAt image: URL) throws {
        try self.init(image: MLFeatureValue(imageAt: image, pixelsWide: 224, pixelsHigh: 224, pixelFormatType: kCVPixelFormatType_32BGRA, options: nil).imageBufferValue!)
    }
...
class Resnet50Output: MLFeatureProvider {
    ...
    var classLabelProbs: [String: Double] {
        return provider.featureValue(for: "classLabelProbs")!.dictionaryValue as! [String: Double]
    }

    /// Most likely image category as string value
    var classLabel: String {
        return provider.featureValue(for: "classLabel")!.stringValue
    }
    ...
    init(classLabelProbs: [String: Double], classLabel: String) {
        provider = try! MLDictionaryFeatureProvider(dictionary: ["classLabelProbs": MLFeatureValue(dictionary: classLabelProbs as [AnyHashable: NSNumber]), "classLabel": MLFeatureValue(string: classLabel)])
    }
...
class Resnet50 {
    ...
    class func load(configuration: MLModelConfiguration = MLModelConfiguration(), completionHandler handler: @escaping (Swift.Result<Resnet50, Error>) -> Void) {
        return load(contentsOf: urlOfModelInThisBundle, configuration: configuration, completionHandler: handler)
    }
    ...
    class func load(configuration: MLModelConfiguration = MLModelConfiguration()) async throws -> Resnet50 {
        return try await load(contentsOf: urlOfModelInThisBundle, configuration: configuration)
    }
